...
def run (eq,timeout,ploc,wd):
    ...
    myerror = ""
    try:
        out = subprocess.check_output ([path,"smt.string_solver=z3str3","dump_models=true","smt.str.fixed_length_iterations=5","smt.str.pre_milliseconds=2000",smtfile],timeout=timeout,stderr=subprocess.STDOUT).decode().strip()
