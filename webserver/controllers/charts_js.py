...
class ChartControllerJS:
    ...
    def generateCactus(self,params):
        ...
        labels = "labels: " + str(list(rdata.keys()))
        series = "series: " + str([list(rdata[s]) for s in rdata])
        data = "" + labels + ",\n" + series + "\n "

        outputstr+="var data = {" + data + "};"

        outputstr+="var options = { showPoint: false,lineSmooth: false, low: 0,showArea: true,axisX: {showGrid: false, showLabel: true,offset: 100}, axisY: {offset: 60,labelInterpolationFnc: function(value) {return '' + value + 'ms';}}};"
    ...
    def generateDistribution(self,params):
        ...
        labels = "labels: " + str([[str(s)+'\nsat','unsat','unknow'] for s in rdata]) #str(list(rdata.keys()))
        ...
        outputstr+="var options = {seriesBarDistance: 15 };"
